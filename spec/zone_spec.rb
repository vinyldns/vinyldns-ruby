...
RSpec.describe Vinyldns::API::Zone do
  test_zone_name = 'testvinyldnsruby'
  first_zone = Vinyldns::API::Zone.search('ingest')['zones'].first

  describe '.connect' do
    it 'can POST & receives 409 Conflict connecting to an already existing zone' do
      expect(Vinyldns::API::Zone.connect(first_zone['name'], first_zone['email'], first_zone['adminGroupId']).class.name).to eq('Net::HTTPConflict')
    end
    it 'raises error when group_id AND group_name_filter are nil arguments' do
      expect { Vinyldns::API::Zone.connect(test_zone_name, first_zone['email']) }.to raise_error(ArgumentError)
    end
    it 'raises error when Group.search is used and group_name doesn\'t find anything' do
      expect { Vinyldns::API::Zone.connect(test_zone_name, first_zone['email'], nil, 'test09999999').message }.to raise_error(ArgumentError, 'No group found for your group_name_filter. Please re-check the spelling so it\'s exact.')
    end
  end
